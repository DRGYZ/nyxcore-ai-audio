...
def _apply(tmp_path, *, rename=False):
    ...
    operations = [ActionPlanOperation('metadata', 'write_metadata', path=str(source), fields=['title'], values={'title': 'Song'}, apply_supported=True, expected_hash=expected_hash)]
    if rename:
        operations.append(ActionPlanOperation('rename', 'rename_file', path=str(source), destination_path=str(tmp_path / 'renamed.mp3'), apply_supported=True, expected_hash=expected_hash))
...
def test_default_backup_and_restore(tmp_path):
    ...
    undo_operation_batch(batch, review_state=state, allowed_roots=(tmp_path,))
...
def test_restore_preserves_newer_content_after_ledger_reload(tmp_path):
    ...
    changed = undo_operation_batch(batch, review_state=state, allowed_roots=(tmp_path,))
